...
------------------------------------------------------------------------------
local function SetMessageAlert(nChannelType, sKey, fCallback)
  if _tCurrentEncounter then
    local tAlerts = _tMessagesAlerts[nChannelType]
    ...
    if fCallback then
      ...
      tAlerts[msg][self.nId] = fCallback
    elseif tAlerts[msg] then
      tAlerts[msg][self.nId] = nil
...
function UnitPrototype:SetDatachronAlert(sKey, fCallback)
  SetMessageAlert(CHANNEL_DATACHRON, sKey, fCallback)
end

function UnitPrototype:SetNPCSayAlert(sKey, fCallback)
  SetMessageAlert(CHANNEL_NPCSAY, sKey, fCallback)
end
